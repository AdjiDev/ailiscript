...
import time
import sys
from lib.gradient import apply_gradient_to_text, rgb_gradient
...
def send_req_bypass_v3(url, packet, proxies, user_agents):
...
def run_threads_bypass_v2(url, packets, threads, proxies, user_agents):
    ...
    for _ in range(threads):
        thread = threading.Thread(target=send_req_bypass_v3, args=(url, packets, proxies, user_agents))
...
def bypass_test_v3(url, packets, threads, proxies_file, user_agents_file):
...
# bypass_test_v3("http://example.com", packets=50, threads=10, proxies_file='proxies.txt', user_agents_file='user_agents.txt')
